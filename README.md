...
The algorithm assesses each candidate decision by evaluating the final objective after assuming the decision is made and completing the simulation using a greedy algorithm.

To limit the computational effort, the following parameters can be provided:

- `max_rollouts`: the maximum number of rollouts to be made at each decision step. If set to a positive value, only the candidate decisions with the best local evaluations are explored. If set to zero (default), all candidates are evaluated.
